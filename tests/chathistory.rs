...
use slirc_proto::{CapSubCommand, Command};
...
async fn test_chathistory_before() -> anyhow::Result<()> {
    ...
    client1
        .send(Command::JOIN("#history".to_string(), None, None))
        .await?;
    client2
        .send(Command::JOIN("#history".to_string(), None, None))
        .await?;
    ...
    let history_msgs: Vec<_> = messages
        .iter()
        .filter_map(|m| {
    ...
    assert!(
        history_msgs.len() >= 3,
        "Should receive at least the 3 messages sent. Got: {:?}",
        history_msgs
    );
