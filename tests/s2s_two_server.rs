...
async fn expect_oper_success(client: &mut TestClient) -> anyhow::Result<()> {
    let _ = client.recv_until(|msg| {
        matches!(&msg.command, Command::Response(resp, _) if resp.code() == 381)
    }).await?;
    Ok(())
}
...
async fn wait_for_link(client: &mut TestClient, peer_name: &str) -> anyhow::Result<()> {
    ...
    for _ in 0..20 { // 20 attempts * 500ms = 10s timeout
        ...
        let msgs = client.recv_until(|msg| {
            matches!(&msg.command, Command::Response(resp, _) if resp.code() == 219)
        }).await?;
    ...
    Err(anyhow::anyhow!("Timed out waiting for link to {}", peer_name))
}

async fn expect_msg_containing(client: &mut TestClient, substring: &str) -> anyhow::Result<slirc_proto::Message> {
...
async fn setup_s2s_env() -> anyhow::Result<(std::path::PathBuf, TestServer, TestServer, TestClient, TestClient)> {
    ...
    std::fs::write(&config_a_path, format!(r#"
[server]
name = "server-a.test"
network = "TestNet"
sid = "001"
...
sid = "002"
autoconnect = false
"#, port_a_client, port_a_s2s, db_a.display(), port_b_s2s))?;

    // Server B config
    let config_b_path = test_dir.join("server_b.toml");
    std::fs::write(&config_b_path, format!(r#"
[server]
name = "server-b.test"
network = "TestNet"
sid = "002"
...
sid = "001"
autoconnect = false
"#, port_b_client, port_b_s2s, db_b.display(), port_a_s2s))?;
    ...
    let mut client_a = TestClient::connect(&format!("127.0.0.1:{}", port_a_client), "alice").await?;
