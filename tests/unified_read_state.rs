...
async fn test_read_marker_sync() {
    ...
    client
        .send(Command::NICK("alice".to_string()))
        .await
    ...
    if let Command::CAP(_, slirc_proto::CapSubCommand::LS, arg1, arg2) = msg.command {
        ...
        assert!(
            caps.contains("draft/read-marker"),
            "Server did not advertise draft/read-marker in: {}",
            caps
        );
    } else {
    ...
    match msg.command {
        ...
        | Command::CAP(_, slirc_proto::CapSubCommand::ACK, _, Some(caps)) => {
            assert!(
                caps.contains("draft/read-marker"),
                "Server did not ACK draft/read-marker"
            );
        }
    ...
    client
        .send_raw(&tag_msg.to_string())
        .await
        .expect("Failed to send TAGMSG");
