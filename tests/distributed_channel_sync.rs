...
fn test_crdt_key_convergence_lww() {
    ...
    // A sets key "foo" at T1
    let t1 = HybridTimestamp::new(101, 0, &sid_a);
    chan_a.key.update(Some("foo".to_string()), t1);

    // B sets key "bar" at T2
    let t2 = HybridTimestamp::new(102, 0, &sid_b);
    chan_b.key.update(Some("bar".to_string()), t2);

    // Merge B into A (B should win)
    chan_a.merge(&chan_b);
    assert_eq!(chan_a.key.value().as_deref(), Some("bar"));

    // Merge A into B (B should stay)
    chan_b.merge(&chan_a);
    assert_eq!(chan_b.key.value().as_deref(), Some("bar"));
}
...
fn test_crdt_boolean_mode_union() {
    ...
    // A sets +n at T1
    let t1 = HybridTimestamp::new(101, 0, &sid_a);
    chan_a.modes.no_external.update(true, t1);

    // B sets +t at T2
    let t2 = HybridTimestamp::new(102, 0, &sid_b);
    chan_b.modes.topic_ops_only.update(true, t2);

    // Merge B into A
    chan_a.merge(&chan_b);
    assert!(*chan_a.modes.no_external.value(), "Should have +n");
    assert!(*chan_a.modes.topic_ops_only.value(), "Should have +t");

    // Merge A into B
    chan_b.merge(&chan_a);
    assert!(*chan_b.modes.no_external.value(), "Should have +n");
    assert!(*chan_b.modes.topic_ops_only.value(), "Should have +t");
}
