...
use crate::handlers::{
    Context, Registry, ResponseMiddleware, WelcomeBurstWriter, labeled_ack, process_batch_message,
    with_label,
};
            ...
            match self.transport.next().await {
                Some(Ok(msg_ref)) => {
                    ...
                    if unreg_state.can_register() && !self.matrix.users.contains_key(&self.uid) {
                        // Use WelcomeBurstWriter to write directly to transport.
                        // This avoids bounded channel deadlock for large MOTDs.
                        let writer = WelcomeBurstWriter::new(
                            &self.uid,
                            &self.matrix,
                            &mut self.transport,
                            &unreg_state,
