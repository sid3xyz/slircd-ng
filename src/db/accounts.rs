...
mod tests {
    ...
    async fn test_verify_password_incorrect() {
        ...
        assert_eq!(
            verify_password(wrong_password.to_string(), hash.clone()).await,
            Ok(false),
    ...
    async fn test_verify_password_empty_password() {
        ...
        assert_eq!(
            verify_password(password.to_string(), hash.clone()).await,
            Ok(true),
            "empty password should verify against its own hash"
        );
        assert_eq!(
            verify_password("nonempty".to_string(), hash.clone()).await,
            Ok(false),
    ...
    async fn test_hash_password_very_long() {
        ...
        assert!(
            verify_password(password.clone(), hash.clone()).await.is_ok(),
