...
impl HistoryProvider for RedbProvider {
    ...
    async fn query_targets(&self, start: i64, end: i64, limit: usize, nick: String, channels: Vec<String>) -> Result<Vec<(String, i64)>, HistoryError> {
        ...
        loop {
            let mut range = table.range(cursor.as_str()..).map_err(|e| HistoryError::Database(e.to_string()))?;
            ...
            if let Some(display_name) = display_target {
                ...
                let mut sub_range = table.range(prefix_start.as_str()..prefix_end.as_str())
