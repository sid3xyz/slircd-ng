...
//! - CHGHOST: Change a user's displayed username/hostname
...
impl Handler for ChghostHandler {
    async fn handle(&self, ctx: &mut Context<'_>, msg: &MessageRef<'_>) -> HandlerResult {
        ...
        // Get target user's current info and channels, then update
        ...
        // Build CHGHOST message: :oldnick!olduser@oldhost CHGHOST newuser newhost
        ...
        // Broadcast to all shared channels - only to clients with chghost capability
        for channel_name in &channels {
            ctx.matrix
                .broadcast_to_channel_with_cap(
                    ...
                    None,
                    Some("chghost"),
                    None, // No fallback - clients without chghost capability get nothing
                )
                .await;
        }

        // Also send to the target user directly if they have chghost cap
...
impl Handler for TraceHandler {
    async fn handle(&self, ctx: &mut Context<'_>, msg: &MessageRef<'_>) -> HandlerResult {
        ...
        // TRACE requires oper privileges
        let Ok(oper_nick) = require_oper(ctx).await else {
            return Ok(());
        };
        ...
        // If a specific target nick is given, show just that user
        if let Some(target_nick) = target {
            if let Some(target_uid) = resolve_nick_to_uid(ctx, target_nick) {
                if let Some(user_ref) = ctx.matrix.users.get(&target_uid) {
                    ...
                    // RPL_TRACEOPERATOR/USER: <nick> <class> <nick>
                    let reply = server_reply(
                        server_name,
                        numeric,
                        vec![
            ...
            // No target: list all connected users
            for user_entry in ctx.matrix.users.iter() {
                let user = user_entry.read().await;
