...
/// Handle WHO for a specific channel.
async fn handle_channel_who(
...
/// Handle WHO for a mask pattern (nick/host matching).
async fn handle_mask_who(
    ctx: &mut Context<'_, RegisteredState>,
    mask_str: &str,
    whox: Option<&WhoxFields>,
    operators_only: bool,
    _multi_prefix: bool, // Unused for mask WHO - no channel prefix context
) -> HandlerResult {
    let server_name = ctx.server_name();
    let nick = &ctx.state.nick;
